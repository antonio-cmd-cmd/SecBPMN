...
def generate_mitigated_bpmn_dual_llm(
    ...
    Args:
        ...
        max_iterations: Optional override for MAX_LLM_ITERATIONS from config.
                        If None, the value from the .env file is used.
        
    Returns:
        Dictionary with generation results including iteration history
    """
    try:
        effective_max_iterations = max_iterations if max_iterations is not None else MAX_LLM_ITERATIONS
        print("=== STARTING DUAL-LLM MITIGATED BPMN GENERATION ===")
        print(f"Max iterations configured: {effective_max_iterations} ({'from request' if max_iterations is not None else 'from .env'})")
        
        # Create fresh LLM instances for this invocation.
        # New instances are created on every call to this function, so no conversation
        # history or internal state is ever shared between separate sweep runs.
        # For Ollama, the server-side KV-cache lifetime is controlled by OLLAMA_KEEP_ALIVE
        # in .env (set to "0" to force model unload after each request for full isolation).
        print(f"[Context isolation] Creating new Generator and Validator LLM instances for this run.")
        ...
        for iteration in range(1, effective_max_iterations + 1):
            ...
            try:
                ...
                # If there are more iterations left, feed back the error and retry
                if iteration < effective_max_iterations:
                    print(f"[Iteration {iteration}] Retrying in next iteration ({iteration + 1}/{effective_max_iterations})...")
                    validation_feedback = (
                        f"Your previous response did not contain valid XML.\n"
                        f"Error: {error_msg}\n"
                        f"Please respond with ONLY the complete BPMN XML wrapped in ```xml``` code blocks. "
                        f"Do not include any explanation outside the XML block."
                    )
                    continue

                # Last iteration exhausted — return best available BPMN (prefer validated ones)
                print(f"[Iteration {iteration}] Max iterations reached with XML extraction error.")
                for prev_iter in reversed(iteration_history):
                    if prev_iter.get('generated_bpmn') and prev_iter.get('status') == 'valid':
                        print(f"Returning validated BPMN from iteration {prev_iter['iteration']}")
                        return {
                            'success': True,
                            'message': f'Returning best validated result from iteration {prev_iter["iteration"]}',
                            'mitigated_bpmn': prev_iter['generated_bpmn'],
                            'iterations': iteration_history,
                            'final_iteration': prev_iter['iteration']
                        }
                # No valid BPMN was ever produced
                return {
                    'success': False,
                    'message': error_msg,
                    'iterations': iteration_history
